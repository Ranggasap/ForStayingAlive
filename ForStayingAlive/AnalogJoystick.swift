...
open class AnalogJoystickComponent: SKSpriteNode {
    ...
    //MARK: - DESIGNATED
    init(diameter: CGFloat, borderColor: UIColor = .gray, fillColor: UIColor = .white, image: UIImage? = nil) {
...
open class AnalogJoystickSubstrate: AnalogJoystickComponent {
    // Inherits everything from AnalogJoystickComponent
}
...
open class AnalogJoystick: SKNode {
    ...
    convenience init(diameters: (substrate: CGFloat, stick: CGFloat?), colors: (substrateBorder: UIColor?, substrateFill: UIColor?, stick: UIColor?)? = nil, images: (substrate: UIImage?, stick: UIImage?)? = nil) {
        let stickDiameter = diameters.stick ?? diameters.substrate * 0.6,
            jColors = colors ?? (substrateBorder: nil, substrateFill: nil, stick: nil),
            jImages = images ?? (substrate: nil, stick: nil),
            substrate = AnalogJoystickSubstrate(diameter: diameters.substrate, borderColor: jColors.substrateBorder ?? .gray, fillColor: jColors.substrateFill ?? .white, image: jImages.substrate),
            stick = AnalogJoystickStick(diameter: stickDiameter, borderColor: jColors.stick ?? .gray, fillColor: jColors.stick ?? .gray, image: jImages.stick)
        self.init(substrate: substrate, stick: stick)
    }
    
    convenience init(diameter: CGFloat, colors: (substrateBorder: UIColor?, substrateFill: UIColor?, stick: UIColor?)? = nil, images: (substrate: UIImage?, stick: UIImage?)? = nil) {
    ...
    @objc func listen() {
        if tracking {
            var scaledData = data
            scaledData.velocity.x *= speedFactor
            scaledData.velocity.y *= speedFactor
            trackingHandler?(scaledData)
        }
    }
